 ...
 * Temporary — remove after production quotes are confirmed restored.
 */
'use strict';
...
function asRegex(spec) {
  if (!spec) return null;
  if (spec.modelMatch && typeof spec.modelMatch === 'object' && spec.modelMatch.__regex) {
    return new RegExp(spec.modelMatch.__regex, spec.modelMatch.__flags || '');
  }
  return null;
}
...
function findModel(dist, spec) {
  ...
  const re = asRegex(spec);
  if (re) {
    const hit = models.find((m) => m && re.test(String(m.name || '')) && m.active !== false);
    if (hit) return hit;
  }
  const label = String((spec && spec.modelLabel) || '').trim().toLowerCase().replace(/\s+hl\b/, '').trim();
  if (label) {
    const hit = models.find((m) => String(m.name || '').toLowerCase().includes(label));
    if (hit) return hit;
  }
...
/**
 * Merge missing PDF-restored prospects/quotations into a distribution blob.
 * Idempotent by email / quote number / OLR ref.
 */
function mergeRecoveryIntoDistribution(distRaw) {
  const dist = distRaw && typeof distRaw === 'object' ? distRaw : {};
  if (!Array.isArray(dist.potentialClients)) dist.potentialClients = [];
  if (!Array.isArray(dist.quotations)) dist.quotations = [];
  if (!dist.settings || typeof dist.settings !== 'object') dist.settings = {};

  const result = { addedQuotes: 0, addedProspects: 0, skippedQuotes: 0, packId: PACK.id };
...
/**
 * Read distribution_data, merge missing PDF quotes, write back if needed.
 */
async function restoreMissingPdfQuotes(pool) {
  ...
  if (!result.addedQuotes && !result.addedProspects) {
    console.log('PDF restore: no missing quotes to add (already present or empty pack).');
