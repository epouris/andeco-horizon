...
async function savePayload(pool, data, opts) {
  ...
  try {
    ...
    if (payKeys.length === 0 && existingPayCount > 0) {
      // Keep existing payslips — empty payroll payload must not wipe history.
    } else if (payKeys.length > 0 && payKeys.length < existingPayCount) {
      // Thinner client cache (e.g. localStorage quota): upsert only, do not delete others.
      ...
      // Full/equal-or-larger payload — safe to replace (supports intentional deletes).
      await replaceTable(
        client, 'payslips',
        ['pay_key', 'employee_id', 'employee_name', 'month', 'year', 'pay_date', 'payroll_number', 'saved_at', 'data'],
        payRows
