...
async function savePayload(pool, data, opts) {
  ...
  try {
    ...
    await replaceTable(
      client,
      'service_reports',
      ['id', 'report_no', 'report_date', 'client_id', 'employee_id', 'notes', 'status', 'created_at', 'updated_at'],
      (Array.isArray(d.serviceReports) ? d.serviceReports : []).filter((r) => r && r.id).map((r) => [
        str(r.id), str(r.reportNo), str(r.reportDate), str(r.clientId), str(r.employeeId), str(r.notes),
